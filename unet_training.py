...
test_image_flair=nib.load('input/BraTS20_Training_001/BraTS20_Training_001_flair.nii').get_fdata()
print(test_image_flair.max())
#Scalers are applied to 1D so let us reshape and then reshape back to original shape.
test_image_flair=scaler.fit_transform(test_image_flair.reshape(-1, test_image_flair.shape[-1])).reshape(test_image_flair.shape)


test_image_t1=nib.load('input/BraTS20_Training_001/BraTS20_Training_001_flair.nii').get_fdata()
test_image_t1=scaler.fit_transform(test_image_t1.reshape(-1, test_image_t1.shape[-1])).reshape(test_image_t1.shape)

test_image_t1ce=nib.load('input/BraTS20_Training_001/BraTS20_Training_001_t1ce.nii').get_fdata()
test_image_t1ce=scaler.fit_transform(test_image_t1ce.reshape(-1, test_image_t1ce.shape[-1])).reshape(test_image_t1ce.shape)

test_image_t2=nib.load('input/BraTS20_Training_001/BraTS20_Training_001_t2.nii').get_fdata()
test_image_t2=scaler.fit_transform(test_image_t2.reshape(-1, test_image_t2.shape[-1])).reshape(test_image_t2.shape)

test_mask=nib.load('input/BraTS20_Training_001/BraTS20_Training_001_seg.nii').get_fdata()
...
#
# imsave('input/combined255.tif', combined_x)
# np.save('input/combined255.npy', combined_x)
#Verify image is being read properly
#my_img=imread('BraTS2020_TrainingData/combined255.tif')
#
# my_img=np.load('input/combined255.npy')
#
...
t1_list = sorted(glob.glob('input/*/*t1.nii'))
t2_list = sorted(glob.glob('input/*/*t2.nii'))
t1ce_list = sorted(glob.glob('input/*/*t1ce.nii'))
flair_list = sorted(glob.glob('input/*/*flair.nii'))
mask_list = sorted(glob.glob('input/*/*seg.nii'))
...
for img in range(len(t2_list)):  # Using t1_list as all lists are of same size
    ...
    if (1 - (counts[0] / counts.sum())) > 0.01:  # At least 1% useful volume with labels that are not 0
        ...
        np.save('input_data_3channel/images/image_' + str(img) + '.npy', temp_combined_images)
        np.save('input_data_3channel/masks/mask_' + str(img) + '.npy', temp_mask)
    else:
        print("I am useless")
...
input_folder = 'input_data_3channel/'
